...
function [CP2,Disp,Std_Clusters,idx,d,sfrac] = Run_Clustering_PCA(XONn,n_clusters,mask,brain_info,maskP,maskN,n_rep,idx_sep_seeds,SeedType)
    ...
    %XONn_filtered = CAP_mask4kmeans(XONn,maskP,maskN,6,mask,brain_info);%Samantha commented out
    ...
    %[idx,CP] = kmeans(XONn_filtered',n_clusters,'distance','correlation','replicates',n_rep,'empty','drop','maxiter',100);
    [idx,CP] = kmeans(XONn',n_clusters,'distance','correlation','replicates',n_rep,'empty','drop','maxiter',300); %Samantha changed
